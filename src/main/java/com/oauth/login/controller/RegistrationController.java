...
public class RegistrationController {
    ...
    public ResponseEntity<?> sendVerificationCode(@RequestParam("email") String email) {
        log.info("\n=== /api/auth/send-verification 요청 수신 ===");
        log.info("요청 파라미터 - email: {}", email);
        
        if (email == null || email.isEmpty()) {
            log.error("오류: 이메일이 비어있습니다.");
            return ResponseEntity.badRequest().body(Collections.singletonMap("error", "이메일을 입력해주세요."));
        }
        
        try {
            log.info("이메일 중복 확인 중...");
            // Check if email already exists and is verified
            if (userRepository.existsByEmailAndIsVerifiedTrue(email)) {
                log.error("오류: 이미 가입된 이메일 주소입니다.");
                return ResponseEntity.badRequest()
                    .body(Collections.singletonMap("error", "이미 가입된 이메일 주소입니다."));
            }
            
            log.info("인증 이메일 전송 시도...");
            // Send verification email
            String token = registrationService.sendVerificationEmail(email);
            log.info("인증 이메일 전송 성공. 토큰: {}", token);
            ...
            log.error("오류 발생 (IllegalStateException): {}", e.getMessage(), e);
            return ResponseEntity.badRequest()
